...
type CreateHandler interface {
	CreateResource(db *gorm.DB, user User, attributes interface{}) error
}

type UpdateHandler interface {
	UpdateResource(db *gorm.DB, attributes interface{}) error
}

type ReloadHandler interface {
	ReloadResource(db *gorm.DB) error
}

type DeleteHandler interface {
	DeleteResource(db *gorm.DB) error
}
...
func New(scoped func(*gorm.DB) *gorm.DB, single func() DBModel, collection func() interface{}, linker func(id uint) string) Resource {
	...
	r.Post = func(ctx *gin.Context, user User, parent DBModel) {
		...
		h, ok := s.(CreateHandler)

		if ok {
			err = h.CreateResource(db, user, s)
		} else {
	...
	r.Get = func(ctx *gin.Context, s DBModel) {
		...
		h, ok := s.(ReloadHandler)

		if ok {
			if err := h.ReloadResource(db); err != nil {
				panic(errors.Wrap(err, "error reloading resource"))
			}
		}
	...
	r.Patch = func(ctx *gin.Context, s DBModel) {
		...
		h, ok := s.(UpdateHandler)

		if ok {
			err = h.UpdateResource(db, newS)
		} else {
			err = db.Model(s).Updates(newS).Error
		}
	...
	r.Delete = func(ctx *gin.Context, s DBModel) {
		...
		h, ok := s.(DeleteHandler)

		if ok {
			err = h.DeleteResource(db)
		} else {
...
func mustGetDB(ctx *gin.Context) *gorm.DB {
	return appkitdb.MustGetGorm(ctx.Request.Context())
}
